const { validateUser, validateAuth } = require('../../../middlewares/validate-user')

describe('validateUser', () => {

  let req, res, next
  const exec = () => { validateUser(req, res, next) }
  beforeEach(() => {
    req = { body: {} }
    res = { status: jest.fn().mockReturnThis(), json: jest.fn() }
    next = (error) => {
      if (error) { res.status = error.status; res.json = error.message } else { res.status = 200; res.json = 'OK' }
    }
  })
...
describe('validateAuth', () => {
  ...
  let req, res, next
  const exec = () => { validateAuth(req, res, next) }
  beforeEach(() => {
    req = { body: {} }
    res = { status: jest.fn().mockReturnThis(), json: jest.fn() }
    next = (error) => {
