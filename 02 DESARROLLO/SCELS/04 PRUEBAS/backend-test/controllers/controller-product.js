...
async function getProductById(req, res, next) {
  ...
  if (!product) return next(createError(404, 'The product with the given ID was not found.'))
  res.json(product)
}

async function createProduct(req, res, next) {
  let category = await Category.findById(req.body.category)
  if (!category) return next(createError(404, 'The category with the given ID was not found.'))
